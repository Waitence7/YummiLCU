...
function render() {
  ...
  <div class="recent"><b>최근 경기</b>${recent ? ` <span>${esc(String(recent.champion))} · ${recent.win ? '승리' : '패배'} · ${recent.kills ?? 0}/${recent.deaths ?? 0}/${recent.assists ?? 0} · ${Math.floor((recent.duration ?? 0)/60)}분</span>` : ' <span>League Client 연결 후 확인할 수 있습니다.</span>'}<button id="recent">새로고침</button></div>
