  ...
  return (
    <div className="space-y-3">
      ...
      <fieldset disabled className="space-y-3 opacity-45">
        ...
        <Card title="오디오 장치">
          <div className="grid grid-cols-2 gap-3">
            <label className="block">
              <span className="mb-1 block text-[12px] text-slate-600">마이크</span>
              <DeviceSelect placeholder="기본 마이크" />
            </label>
            <label className="block">
              <span className="mb-1 block text-[12px] text-slate-600">출력</span>
        ...
        <Card title="참가자">
          <p className="text-[12px] text-slate-500">방에 참가하면 참가자 목록이 표시됩니다.</p>
        </Card>
      </fieldset>
    </div>
  );
...
function DeviceSelect({ placeholder }: { placeholder: string }) {
  return (
    <select className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-[12px] text-slate-700">
