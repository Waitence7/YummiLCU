...
export const TRAY_HIDE_EFFECT_OPTIONS: ReadonlyArray<{
  ...
  { value: 'book-return-v2', label: '유미의 책 · 귀환 V2', description: '처음부터 표지와 책등을 가진 입체 책으로 창이 접히고, 두꺼운 페이지 단면을 보이며 트레이로 떨어집니다.' },
  ...
  try {
    switch (effect) {
      ...
      case 'book-return':
        ...
        break;
      case 'book-return-v2':
        if (!(await playHtmlCanvasTrayEffect(surface, 'book-return-v2', cleanup, activePlaybackRate))) {
          if (!(await playHtmlCanvasTrayEffect(surface, 'page-curl', cleanup, activePlaybackRate))) await fold(surface);
        }
        break;
      case 'curtain':
