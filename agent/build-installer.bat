...
if exist "agent.json" (
  copy /Y "agent.json" "YummiLcu.Agent\bin\Release\net8.0-windows\win-x64\publish\agent.json" >nul
) else (
  copy /Y "agent.json.example" "YummiLcu.Agent\bin\Release\net8.0-windows\win-x64\publish\agent.json" >nul
)

for /f "usebackq tokens=*" %%V in (`powershell -NoProfile -Command "[xml]$x=Get-Content 'YummiLcu.Agent\YummiLcu.Agent.csproj'; $x.Project.PropertyGroup.Version"`) do set APP_VER=%%V
...
set "PUB=YummiLcu.Agent\bin\Release\net8.0-windows\win-x64\publish"
