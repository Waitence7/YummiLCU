...
internal readonly record struct ActionContext(LcuClient Lcu, AgentConfig Config, JsonElement? Payload);

/// <summary>WS command action whitelist — relay/actions.py 와 동일하게 유지.</summary>
internal static class AllowedActions
{
  private static readonly Dictionary<string, Func<ActionContext, Task<(bool Ok, string Message)>>> Handlers =
      new(StringComparer.Ordinal)
      {
          ...
          ["accept_match"] = async ctx => Bool(await ctx.Lcu.PostAsync("/lol-matchmaking/v1/ready-check/accept")),
          ["decline_match"] = async ctx => Bool(await ctx.Lcu.PostAsync("/lol-matchmaking/v1/ready-check/decline")),
          ["reconnect"] = async ctx => Bool(await ctx.Lcu.PostAsync("/lol-gameflow/v1/reconnect")),
          ["dodge"] = DodgeAsync,
          ["queue_start"] = async ctx => Bool(await ctx.Lcu.PostAsync("/lol-lobby/v2/lobby/matchmaking/search")),
          ["queue_cancel"] = async ctx => Bool(await ctx.Lcu.DeleteAsync("/lol-lobby/v2/lobby/matchmaking/search")),
          ["leave_lobby"] = async ctx => Bool(await ctx.Lcu.DeleteAsync("/lol-lobby/v2/lobby")),
          ["party_ready"] = async ctx => Bool(await ctx.Lcu.PutJsonAsync("/lol-lobby/v1/parties/ready", """{"ready":true}""")),
          ["champ_reroll"] = async ctx => Bool(await ctx.Lcu.PostAsync("/lol-champ-select/v1/session/my-selection/reroll")),
          ["quit_client"] = async ctx => Bool(await ctx.Lcu.PostAsync("/process-control/v1/process/quit")),
  ...
  private static string? PayloadString(JsonElement? payload, string key)
  {
    if (payload is null || payload.Value.ValueKind != JsonValueKind.Object)
      return null;
    if (!payload.Value.TryGetProperty(key, out var el))
      return null;
    return el.GetString();
  }
}
